...
int main()
{
	...
	bool mouseSelecting = false, moving = false, cond = false;
	...
	std::vector<std::shared_ptr<Piece>> pieceList = Main::generatePositionFromFENID("4k3/8/8/8/4n3/8/p2P4/K6R b - - 0 1",
	...
	while (window.isOpen())
	{
		...
		for (auto it2 = pieceList.begin(); it2 != pieceList.end(); it2++) {
			...
			if (piece->targetPos.has_value()) {
				if (piece->getGlobalPosition() == piece->targetPos.value()) {
					...
					moving = true;
					piece->setGlobalPosition(Main::Interpolate(piece->getGlobalPosition(), piece->targetPos.value(), 0.005f), true);
				}
			}
		}
		...
		if (cond) {
			for (auto& piece : pieceList) {
				if (piece != nullptr && piece->captured) {
					size_t ind = piece->index;
					piece.reset();
					pieceList.erase(pieceList.begin() + ind);
					cond = false;
					break;
				}
			}
		...
		window.clear();
		window.draw(board);
		if (selectedPiece != nullptr && !moving) {
		...
		if (selectedPiece != nullptr) {
			...
			selectedPieceBackground.setPosition(selectedPiece->getGlobalPosition());
