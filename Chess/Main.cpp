...
int main()
{
	...
	Board board{ Standard, FEN, boardSize, ANIMATED, window, 1, BOARD_SET, PIECE_SET, pieceSpriteSheets, boardSpriteSheet, boardTexture, selectedPieceTexture, checkTexture, lastMoveTexture, kothBackgroundTexture, kothShadowTexture, rankBackgroundTexture, rankShadowTopTexture, checkT, textFont, AI, WHITE, CHESS960, ENDGAME, pieceTextures, extraTextures};
	...
	while (window.isOpen())
	{
		...
		selectedPos = { std::ceil(((float)mousePos.x - board.boardXOffset) / board.boardMultiplier),  9 - std::ceil((float)mousePos.y / board.boardMultiplier) };
			...
			if (!board.pieceSelectionLock) {
				for (auto& piece : board.pieceList) {
					if (piece->name == "King" && piece->isWhite()) {
						...
						board.whiteCheckText.setPosition({ board.whiteCheckCount.getPosition().x - 10.0f, board.whiteCheckCount.getPosition().y - 20.0f });
						break;
					}
				}
				for (auto& piece : board.pieceList) {
					if (piece->name == "King" && !piece->isWhite()) {
						...
						board.blackCheckText.setPosition({ board.blackCheckCount.getPosition().x - 10.0f, board.blackCheckCount.getPosition().y - 20.0f });
						break;
					}
				}
			}
		...
		if (!board.calculatingPos) {
			if (board.StockfishEnabled && board.whiteToPlay != board.playerSideWhite && board.animationFinished) {
			...
			if (!sf::Mouse::isButtonPressed(sf::Mouse::Button::Left) && board.pieceSelectionLock) {
						...
						if (sprite.getGlobalBounds().contains((sf::Vector2f)mousePos)) {
							...
							if (board.selectedPiece->name == "Pawn") {
								if (board.selectedPiece->color == PColor::White) {
									if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).y == 8) {
										board.promotePiece = board.selectedPiece;
										board.promotePiece->setTarget(sprite.getPosition());
									}
									else if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).y == board.selectedPiece->getLocalPosition().y + 2) {
									...
									if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).y == 1) {
										board.promotePiece = board.selectedPiece;
										board.promotePiece->setTarget(sprite.getPosition());
									}
									else if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).y == board.selectedPiece->getLocalPosition().y - 2) {
							...
							if (board.promotePiece == nullptr) {
								...
								board.currentMove += Main::convertPositiontoNotation(Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier));
								board.moves += " " + board.currentMove;
								board.lastMoveStart.setPosition(Main::getGlobalPosition(board.selectedPiece->getLocalPosition(), board.boardXOffset, board.boardMultiplier));
								board.lastMoveDest.setPosition(sprite.getPosition());
								board.selectedPiece->setGlobalPosition(sprite.getPosition());
								board.selectedPiece->setGhostSpriteVisible(false, true);
								board.calculatingPos = true;
								std::thread postMoveF(Main::postMove, board.selectedPiece, std::ref(board), extraTextures);
					...
					if (board.selectedPiece->name == "King") {
						...
						for (auto& sprite : king->castleSquares) {
							if (sprite.getGlobalBounds().contains(sf::Vector2f(mousePos))) {
								...
								if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == 3) {
									std::shared_ptr<Piece> rook = Main::getPieceFromPosition({ 1, king->getLocalPosition().y }, board.pieceList);
									if (rook != nullptr) {
										rook->setGlobalPosition(Main::getGlobalPosition({ 4, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
										king->setGlobalPosition(sprite.getPosition());
										king->setGhostSpriteVisible(false, true);
										board.calculatingPos = true;
										std::thread postCastleF(Main::postCastle, king, rook, std::ref(board), extraTextures);
										board.selectedPiece.reset();
										postCastleF.detach();
									}
								}
								else if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == 7) {
									std::shared_ptr<Piece> rook = Main::getPieceFromPosition({ 8, king->getLocalPosition().y }, board.pieceList);
									if (rook != nullptr) {
										rook->setGlobalPosition(Main::getGlobalPosition({ 6, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
										king->setGlobalPosition(sprite.getPosition());
										king->setGhostSpriteVisible(false, true);
										board.calculatingPos = true;
										std::thread postCastleF(Main::postCastle, king, rook, std::ref(board), extraTextures);
						...
						for (auto& sprite : king->castleCaptureSquares) {
							if (sprite.getGlobalBounds().contains(sf::Vector2f(mousePos))) {
								...
								if (king->isWhite()) {
									if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == board.wKRook) {
										...
										if (rook != nullptr) {
											...
											board.moves += board.currentMove;
											rook->setGlobalPosition(Main::getGlobalPosition({ 6, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
											king->setGlobalPosition(Main::getGlobalPosition({ 7, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
											king->setGhostSpriteVisible(false, true);
											board.calculatingPos = true;
											std::thread postCastleF(Main::postCastle, king, rook, std::ref(board), extraTextures);
											board.selectedPiece.reset();
									...
									else if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == board.wQRook) {
										...
										if (rook != nullptr) {
											...
											board.moves += board.currentMove;
											rook->setGlobalPosition(Main::getGlobalPosition({ 4, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
											king->setGlobalPosition(Main::getGlobalPosition({ 3, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
											king->setGhostSpriteVisible(false, true);
											board.calculatingPos = true;
											std::thread postCastleF(Main::postCastle, king, rook, std::ref(board), extraTextures);
											board.selectedPiece.reset();
									...
									if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == board.bKRook) {
									...
									else if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == board.bQRook) {
					...
					if (board.selectedPiece->name == "Pawn") {
						...
						for (auto& sprite : pawn->enPassantSquares) {
							if (sprite.getGlobalBounds().contains(sf::Vector2f(mousePos))) {
								...
								for (auto& piece : board.pieceList) {
									if (pawn->isWhite()) {
										if (piece->getLocalPosition() == (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier) - sf::Vector2i(0, 1))) {
											board.capturePiece = piece;
											break;
										}
									}
				...
				if (board.selectedPiece != nullptr) {
					if (!board.selectedPieceBackground.getGlobalBounds().contains(board.selectedPiece->getGlobalPosition())) {
						board.selectedPiece->setGlobalPosition(Main::getGlobalPosition(board.selectedPiece->getLocalPosition(), board.boardXOffset, board.boardMultiplier));
						board.selectedPiece.reset();
					}
					else {
						board.selectedPiece->setGlobalPosition(Main::getGlobalPosition(board.selectedPiece->getLocalPosition(), board.boardXOffset, board.boardMultiplier));
				...
				board.selectedPiece->setGlobalPosition(sf::Vector2f(mousePos));
			}
			...
			if (board.promotePiece != nullptr) {
				if (board.unpromoting) {
					if (board.promotePiece->targetPos.has_value()) {
						if (board.promotePiece->getGlobalPosition() == board.promotePiece->targetPos.value()) {
							board.promotePiece->setLocalPosition(Main::getLocalPosition(board.promotePiece->getGlobalPosition(), board.boardXOffset, board.boardMultiplier));
							board.promotePiece->setTarget({});
							board.promotePiece.reset();
							board.unpromoting = false;
						}
						else {
							board.promotePiece->setGlobalPosition(Main::Interpolate(board.promotePiece->getGlobalPosition(), board.promotePiece->targetPos.value(), 0.25f));
						}
					}
				}
				else {
					if (board.promotePiece->targetPos.has_value()) {
						if (board.promotePiece->getGlobalPosition() == board.promotePiece->targetPos.value()) {
							...
							int x = Main::getLocalPosition(board.promotePiece->getGlobalPosition(), board.boardXOffset, board.boardMultiplier).x;
							if (board.promotePiece->isWhite()) {
								for (int y = 8; y >= 5 - off; y--) {
									...
									bg.setPosition(Main::getGlobalPosition({ x, y }, board.boardXOffset, board.boardMultiplier));
									...
									case 8:
									{
										...
										piece.setPosition(Main::getGlobalPosition({ x, y }, board.boardXOffset, board.boardMultiplier));
				...
				if (board.castleKing->targetPos.value() != board.castleKing->getGlobalPosition() || board.castleRook->targetPos.value() != board.castleRook->getGlobalPosition()) {
					board.castleKing->setGlobalPosition(Main::Interpolate(board.castleKing->getGlobalPosition(), board.castleKing->targetPos.value(), 0.25f));
					board.castleRook->setGlobalPosition(Main::Interpolate(board.castleRook->getGlobalPosition(), board.castleRook->targetPos.value(), 0.25f));
				}
				else {
					std::shared_ptr<King> king = std::dynamic_pointer_cast<King>(board.castleKing);
					board.calculatingPos = true;
					std::thread postCastleF(Main::postCastle, king, board.castleRook, std::ref(board), extraTextures);
				...
				for (auto it2 = board.pieceList.begin(); it2 != board.pieceList.end(); ) {
					...
					if (piece != nullptr && piece->targetPos.has_value()) {
						if (piece->getGlobalPosition() == piece->targetPos.value()) {
							...
							board.calculatingPos = true;
							std::thread postMoveF(Main::postMove, piece, std::ref(board), extraTextures);
							postMoveF.detach();
							break;
						}
						else {
							...
							if (piece->name == "Pawn") {
								...
								if (pawn->capturingEnPassant) {
									...
									if (board.capturePiece != nullptr) {
										piece->setGlobalPosition(Main::Interpolate(piece->getGlobalPosition(), piece->targetPos.value(), 0.25f, board.threshold));
						...
						if (sprite.getGlobalBounds().contains(sf::Vector2f(mousePos))) {
							if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
								...
								if (Main::getLocalPosition(sprite.getPosition(), board.boardXOffset, board.boardMultiplier).x == 3) {
									std::shared_ptr<Piece> rook = Main::getPieceFromPosition({ 1, king->getLocalPosition().y }, board.pieceList);
									if (rook != nullptr) {
										rook->setTarget(Main::getGlobalPosition({ 4, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
										king->setTarget(Main::getGlobalPosition({ 3, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
									...
									std::shared_ptr<Piece> rook = Main::getPieceFromPosition({ 8, king->getLocalPosition().y }, board.pieceList);
									if (rook != nullptr) {
										rook->setTarget(Main::getGlobalPosition({ 6, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
										king->setTarget(Main::getGlobalPosition({ 7, king->getLocalPosition().y }, board.boardXOffset, board.boardMultiplier));
										board.castleKing = king;
										board.castleRook = rook;
									}
								}
		...
		if (board.selectedPiece != nullptr) {
			board.selectedPieceBackground.setPosition(Main::getGlobalPosition(board.selectedPiece->getLocalPosition(), board.boardXOffset, board.boardMultiplier));
		...
		if (board.promotePiece != nullptr && !board.unpromoting && !board.calculatingPos) {
			...
			for (int i = 0; i < board.promoteBackgrounds.size(); i++) {
				...
				if (sprite.getGlobalBounds().contains(sf::Vector2f(mousePos))) {
					...
					if (sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
						sf::Vector2f globalPos = Main::getGlobalPosition(board.promotePiece->getLocalPosition(), board.boardXOffset, board.boardMultiplier);
						sf::Vector2i localPos = Main::getLocalPosition(board.promotePiece->getGlobalPosition(), board.boardXOffset, board.boardMultiplier);
						...
						case 0:
						{
							std::shared_ptr<Queen> queen = std::make_shared<Queen>(localPos.x, localPos.y, board.pieceScale, board.boardXOffset, board.boardMultiplier, color, board.pieceTextures.at(10 + static_cast<std::vector<std::reference_wrapper<sf::Texture>, std::allocator<std::reference_wrapper<sf::Texture>>>::size_type>(offset)), false);
							...
							board.moves += board.currentMove;
							board.calculatingPos = true;
							std::thread postMoveF(Main::postMove, queen, std::ref(board), extraTextures);
							postMoveF.detach();
							break;
						}
						case 1:
						{
							std::shared_ptr<Knight> knight = std::make_shared<Knight>(localPos.x, localPos.y, board.pieceScale, board.boardXOffset, board.boardMultiplier, color, board.pieceTextures.at(8 + static_cast<std::vector<std::reference_wrapper<sf::Texture>, std::allocator<std::reference_wrapper<sf::Texture>>>::size_type>(offset)), false);
							...
							board.moves += board.currentMove;
							board.calculatingPos = true;
							std::thread postMoveF(Main::postMove, knight, std::ref(board), extraTextures);
							postMoveF.detach();
							break;
						}
						case 2:
						{
							std::shared_ptr<Rook> rook = std::make_shared<Rook>(localPos.x, localPos.y, board.pieceScale, board.boardXOffset, board.boardMultiplier, color, board.pieceTextures.at(11 + static_cast<std::vector<std::reference_wrapper<sf::Texture>, std::allocator<std::reference_wrapper<sf::Texture>>>::size_type>(offset)), false);
							...
							board.moves += board.currentMove;
							board.calculatingPos = true;
							std::thread postMoveF(Main::postMove, rook, std::ref(board), extraTextures);
							postMoveF.detach();
							break;
						}
						case 3:
						{
							std::shared_ptr<Bishop> bishop = std::make_shared<Bishop>(localPos.x, localPos.y, board.pieceScale, board.boardXOffset, board.boardMultiplier, color, board.pieceTextures.at(6 + static_cast<std::vector<std::reference_wrapper<sf::Texture>, std::allocator<std::reference_wrapper<sf::Texture>>>::size_type>(offset)), false);
							...
							board.moves += board.currentMove;
							board.calculatingPos = true;
							std::thread postMoveF(Main::postMove, bishop, std::ref(board), extraTextures);
							postMoveF.detach();
							break;
						}
						case 4:
						{
							std::shared_ptr<King> king = std::make_shared<King>(localPos.x, localPos.y, board.pieceScale, board.boardXOffset, board.boardMultiplier, color, board.pieceTextures.at(7 + static_cast<std::vector<std::reference_wrapper<sf::Texture>, std::allocator<std::reference_wrapper<sf::Texture>>>::size_type>(offset)), false);
							...
							board.moves += board.currentMove;
							board.calculatingPos = true;
							std::thread postMoveF(Main::postMove, king, std::ref(board), extraTextures);
							postMoveF.detach();
							break;
						}
						}
		...
		if (board.promoting && sf::Mouse::isButtonPressed(sf::Mouse::Button::Left)) {
			...
			board.promotePiece->setTarget(Main::getGlobalPosition(board.promotePiece->getLocalPosition(), board.boardXOffset, board.boardMultiplier));
		}
		if (board.selectedPiece != nullptr) {
		...
		window.draw(board.dropPieceBackground);
		window.display();
	}
}