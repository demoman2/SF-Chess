...
int main()
{
	...
	// Board  
	bool hourGlass = false, makingBoard = false, isFocused = true, controlClicked = false, shiftClicked = false;
	...
	sf::Image bAIGradient = makeAIStrengthTexture(bGUIGradient, boardTexture.copyToImage().getPixel({ 129, 3 }));
	sf::Image sAIGradient = makeAIStrengthTexture(bGUIGradient, boardTexture.copyToImage().getPixel({ 129, 3 }));
	...
	std::vector<sf::Texture> pieceTextures = Chess::makePieceSet(pieceSpriteSheets.at(1), 12, 320);
	std::vector<std::vector<tgui::Texture>> svgSpriteSheets;
	svgSpriteSheets.emplace_back(loadSVGSet("assets/ui/svg/alpha"));
	svgSpriteSheets.emplace_back(loadSVGSet("assets/ui/svg/cburnett"));
	svgSpriteSheets.emplace_back(loadSVGSet("assets/ui/svg/merida"));
	svgSpriteSheets.emplace_back(loadSVGSet("assets/ui/svg/disguised"));
	sf::Clock testClock;
	...
	// Board Customization
	BoardSettings bP, boardSettings;
	StockfishSettings selectedSFSettings, sfSettings;

	// Boards
	// std::shared_ptr<Board> board = std::make_shared<Board>(bP, sfSettings, window.getSize(), pieceSpriteSheets, boardSpriteSheet, textFont, boardTexture, pieceTextures, boardTextures, soundBuffers, true);
	...
	auto newBoard = [&]() {
		if (!makingBoard) {
			...
			makingBoard = true;
			std::thread makeBoardThread(makeBoard, std::ref(selectedBoard), std::cref(boardSettings), std::cref(sfSettings), window.getSize(), std::ref(pieceSpriteSheets), std::ref(boardSpriteSheet), std::ref(textFont), std::ref(boardTexture), std::ref(pieceTextures), std::ref(boardTextures), std::ref(soundBuffers), std::ref(boardList), std::ref(makingBoard));
	...
	auto newBoardWith = [&](const BoardSettings& boardSettings, const StockfishSettings& stockfishSettings) {
		if (!makingBoard) {
			for (auto& board : boardList) { board->mouseModeOff(); }
			makingBoard = true;
			std::thread makeBoardThread(makeBoard, std::ref(selectedBoard), std::cref(boardSettings), std::cref(stockfishSettings), window.getSize(), std::ref(pieceSpriteSheets), std::ref(boardSpriteSheet), std::ref(textFont), std::ref(boardTexture), std::ref(pieceTextures), std::ref(boardTextures), std::ref(soundBuffers), std::ref(boardList), std::ref(makingBoard));
			makeBoardThread.detach();
		}
	};
...
#pragma region Board GUI
	...
	tgui::ComboBox::Ptr bVariantBox = boardGUI.get<tgui::ComboBox>("Variant Box");
	...
	tgui::BitmapButton::Ptr bRandomizeVariant = boardGUI.get<tgui::BitmapButton>("Randomize");
	...
	GUIBoard FENBoard{ bFENImage, positionGUI, svgSpriteSheets.at(1) }, largeFENBoard{ bLargeFENImage, largePositionGUI, svgSpriteSheets.at(1) };
	...
	bFENArea->onTextChange([&](const tgui::String& text) {
		...
		FENBoard.setPieces(bP.FEN.value_or(""), svgSpriteSheets.at(bPieceSet->getText().toInt() - 1), reversed);
		largeFENBoard.setPieces(bP.FEN.value_or(""), svgSpriteSheets.at(bPieceSet->getText().toInt() - 1), reversed);
	});
	bFENArea->onUnfocus([&]() {
	...
	bVariantBox->onItemSelect([&](int index) { bP.variant = static_cast<Chess::Variant>(index); });
	...
	auto setStockfishLevelTexturesB = [&](const sf::Texture& boardT) {
		bAIGradient = makeAIStrengthTexture(bGUIGradient, boardT.copyToImage().getPixel({ 129, 3 }));
	...
	auto onBoardSetChange = [&](int add = 0) {
		...
		sf::Texture nBoardTexture = Chess::loadBoard(boardSpriteSheet, newInt - 1, 1024, 2);
		...
		nBoardTexture = Chess::loadBoard(boardSpriteSheet, newInt - 1, 1024, 8);
		boardTextureT.loadFromPixelData(nBoardTexture.getSize(), nBoardTexture.copyToImage().getPixelsPtr());
	...
	auto onPieceSetChange = [&](int add = 0) {
		int newInt = std::clamp(bPieceSet->getText().toInt() + add, 1, int(pieceSpriteSheets.size()));
		...
		else if (newInt >= pieceSpriteSheets.size()) {
			bPieceSetPlus->setEnabled(false);
			bPieceSetPlusL->getRenderer()->setTextColor(tgui::Color("#9E9E9E"));
		}
		sf::Texture pieceTexture = Chess::loadPieceSet(pieceSpriteSheets.at(newInt - 1), 320);
		...
		FENBoard.setPieces(FEN, svgSpriteSheets.at(newInt - 1), reversed);
		largeFENBoard.setPieces(FEN, svgSpriteSheets.at(bPieceSet->getText().toInt() - 1), reversed);
	...
	bSaveButton->onPress([&]() {
		if (bMode == 0) {
			...
			selectedBoard->loadFromBoardArgs(bP, pieceSpriteSheets, boardSpriteSheet);
			selectedBoard->setStockfishSettings(sfSettings);
		}
		bGUIvisible = false;
	});
	bRandomizeVariant->onPress([&]() {
		int random = rand() % 8;
		bVariantBox->setSelectedItemByIndex(random);
	});
	bAdvanced->onPress([&]() {
		bGUIvisible = false;
	...
	bStartpos->onPress([&]() {
		bFENArea->setText(Chess::getNewFEN(bP.variant, bP.chess960));
	});
	...
	auto loadBoardGUI = [&](int mode, const BoardSettings& boardSettings, const StockfishSettings& stockfishSettings) {
		...
		bVariantBox->setSelectedItemByIndex(static_cast<int>(boardSettings.variant));
...
#pragma region Stockfish GUI
	...
	auto setStockfishLevelTexturesS = [&](const sf::Texture& boardT) {
		sAIGradient = makeAIStrengthTexture(bGUIGradient, boardT.copyToImage().getPixel({ 129, 3 }));
		tgui::Texture texture;
	...
	auto loadStockfishGUI = [&](const BoardSettings& boardSettings, const StockfishSettings& settings, bool fromBoard) {
		...
		if (fromBoard && selectedBoard) {
			setStockfishLevelTexturesS(selectedBoard->getBoardT());
		}
		else if (!fromBoard) {
			setStockfishLevelTexturesS(Chess::loadBoard(boardSpriteSheet, boardSettings.boardSet, 1024));
		}
...
#pragma region FEN GUI
	...
	GUIBoard fBoardGUI(fBoard, fenPositionGUI, svgSpriteSheets.at(1));

	fFENArea->onTextChange([&](const tgui::String& text) {
		bP.FEN = text.toStdString();
		fBoardGUI.setPieces(text.toStdString(), svgSpriteSheets.at(bP.pieceSet));
	});
	...
	fLoadPosition->onClick([&]() { if (selectedBoard) { selectedBoard->loadFromBoardArgs(bP, pieceSpriteSheets, boardSpriteSheet); fGUIvisible = false; } });
	...
	auto loadFENGUI = [&](const BoardSettings& boardSettings) {
		...
		if (selectedBoard) {
			...
			fBoardTexture.loadFromPixelData(selectedBoard->getBoardT().getSize(), selectedBoard->getBoardT().copyToImage().getPixelsPtr());
...
#pragma region Position
	...
	tgui::ComboBox::Ptr aVariantBox = advancedBoardPosition.get<tgui::ComboBox>("Variant Box");
	tgui::BitmapButton::Ptr aRandomVariant = advancedBoardPosition.get<tgui::BitmapButton>("Random Variant");
	...
	GUIBoard aFENBoard(aFENImage, aPositionGUI, svgSpriteSheets.at(1)), aRepeatFENBoard(aRepeatFENImage, aRepeatPositionGUI, svgSpriteSheets.at(1));
	aFENArea->setTabString("/");
	aRepeatFENArea->setTabString("/");
	
	aVariantBox->onItemSelect([&](int index) {
		bP.variant = static_cast<Chess::Variant>(index);
	...
	aRandomVariant->onPress([&]() {
		int random = rand() % 8;
		aVariantBox->setSelectedItemByIndex(random);
	});
	...
	aPositionSet->onItemSelect([&](int i) {
		if (i == 0) {
			...
			switch (bP.variant) {
			case Chess::Standard :
				if (i == 1) { bP.endgamePosition = true; }
				break;
			case Chess::Antichess:
				if (i == 1) { bP.endgamePosition = true; }
				break;
	...
	aFENArea->onTextChange([&](const tgui::String& text) {
		...
		aFENBoard.setPieces(bP.FEN.value_or(""), svgSpriteSheets.at(bP.pieceSet), reversed);
	});
	...
	aRepeatFENArea->onTextChange([&](const tgui::String& text) {
		...
		aRepeatFENBoard.setPieces(bP.newPositionFEN.value_or(""), svgSpriteSheets.at(bP.pieceSet), reversed);
	});
	...
	aStartpos->onPress([&]() {
		aFENArea->setText(Chess::getNewFEN(bP.variant, bP.chess960));
	});

	auto loadAdvancedPosition = [&]() {
		bP.endgamePosition = bP.endgamePosition;
		aVariantBox->setSelectedItemByIndex(static_cast<int>(bP.variant));
...
#pragma region Appearance
	...
	tgui::EditBox::Ptr aPieceSet = advancedBoardAppearance.get<tgui::EditBox>("Piece Set");
	...
	auto onPieceSetChangeA = [&](int add = 0) {
		int newInt = std::clamp(aPieceSet->getText().toInt() + add, 1, int(pieceSpriteSheets.size()));
		...
		aFENBoard.setPieces(FEN, svgSpriteSheets.at(newInt - 1), reversed);
		...
		aRepeatFENBoard.setPieces(FEN, svgSpriteSheets.at(newInt - 1), reversed);
		bP.pieceSet = newInt - 1;
	};

	aBoardSet->onUnfocus(onBoardSetChangeA);
	aPieceSet->onUnfocus(onPieceSetChangeA);
...
#pragma endregion
	...
	setAdvancedBoardTextures = [&](const sf::Texture& boardT) {
		bAIGradient = makeAIStrengthTexture(aGUIGradient, boardT.copyToImage().getPixel({ 129, 3 }));
		tgui::Texture texture;
		texture.loadFromPixelData(bAIGradient.getSize(), bAIGradient.getPixelsPtr());
...
#pragma region Header GUI
	...
	headerClock.reset();
	bool hBoardHover = false, hBoardBoundsHover = false, hEngineHover = false, hEngineBoundsHover = false, hTopHover = false;
	...
	hSelectedBoardSettings->onPress([&]() {
		loadBoardGUI(2, selectedBoard->getBoardArgs(), selectedBoard->getStockfishSettings());
		hBoard->setDown(false);
	});
	...
	hSelectedBoardAdvanced->onPress([&]() {
		loadAdvancedBoard(selectedBoard->getBoardArgs(), selectedBoard->getStockfishSettings(), true, false);
		hBoard->setDown(false);
	});
	hCopyFEN->onPress([&]() {
		if (selectedBoard) { clipboard << selectedBoard->getCurrentFEN(); }
		hBoard->setDown(false);
	});
	hLoadFEN->onPress([&]() {
		if (selectedBoard) { loadFENGUI(selectedBoard->getBoardArgs()); }
		hBoard->setDown(false);
	});
	...
	hSelectedBoardEngineSettings->onPress([&]() {
		loadStockfishGUI(selectedBoard->getBoardArgs(), selectedBoard->getStockfishSettings(), true);
		hEngine->setDown(false);
	});
	...
	hSelectedEngineAdvancedSettings->onPress([&]() {
		loadAdvancedStockfishGUI(selectedBoard->getBoardArgs(), selectedBoard->getStockfishSettings(), true);
		hEngine->setDown(false);
	});
	hShadow->onMouseEnter([&]() { hTopHover = true; });
...
#pragma endregion
	...
	std::vector<sf::Image> windowIcons = makeBoardIcons(boardSpriteSheet, renderTexture, iconTexture);
	window.setIcon(windowIcons.front().getSize(), windowIcons.front().getPixelsPtr());
	deltaClock.restart();

	while (window.isOpen()) {
		guiVisible = (sGUIvisible || bGUIvisible || aBGUIVisible || sGUIvisible);
		guiVisibleFromBoard = (sGUIvisible && sFromBoard) || (bGUIvisible && bMode == 2) || (aBGUIVisible && aFromBoard) || (asGUIvisible && asFromBoard);
		...
		if (hTopHover && !headerVisible && hidHeader) {
		...
		else if (!hTopHover && !hBoard->isDown() && !hEngine->isDown() && headerVisible) {
			for (auto& widget : headerGUI.getWidgets()) {
			...
			if (!makingBoard && !guiVisible) {
				for (auto it = boardList.rbegin(); it != boardList.rend(); it++) {
					...
					if (b && b->intersects(mousePos)) {
						if (b != selectedBoard) {
							sf::Image& icon = windowIcons.at(b->getBoardSet());
		...
		while (const std::optional event = window.pollEvent())
		{
			...
			if (event->is<sf::Event::Closed>())
			{
						...
						if (controlClicked) {
							...
							if (keyPressed->code == sf::Keyboard::Key::F) {
								if (!guiVisible && selectedBoard) {
									loadFENGUI(selectedBoard->getBoardArgs());
								}
							}
							else if (keyPressed->code == sf::Keyboard::Key::D) {
								if (!guiVisible && selectedBoard) {
							...
							if (keyPressed->code == sf::Keyboard::Key::B && !guiVisible) {
								// Set Icon
								if (selectedBoard && !guiVisible && selectedBoard->areKeybindsEnabled()) {
									sf::Image& icon = windowIcons.at((selectedBoard->getBoardSet() + 1) % 24);
									window.setIcon(icon.getSize(), icon.getPixelsPtr());
								}
							}
							else if (keyPressed->code == sf::Keyboard::Key::E && !guiVisible) {
								newBoard();
							}
				...
				guiVisible = (sGUIvisible || bGUIvisible || aBGUIVisible || sGUIvisible);
				if (selectedBoard && !guiVisible) { selectedBoard->handleEvent(event, mousePos, boardSpriteSheet, pieceSpriteSheets, isFocused, controlClicked); }
			}
		}
		...
		if (makingBoard) { window.setMouseCursor(mouseCursors.at(3)); }
			...
			window.setMouseCursor(mouseCursors.at(selectedBoard->getMouseCursor(hourGlass)));
		}
...
{
	std::vector<std::string> pieceNames = { "B", "K", "N", "P", "Q", "R" };
	std::vector<tgui::Texture> textures;
	for (auto& p : pieceNames) {
		tgui::String filePath = (path + "/b" + p + ".svg");
		textures.emplace_back(filePath);
	}
	for (auto& p : pieceNames) {
		tgui::String filePath = (path + "/w" + p + ".svg");
		textures.emplace_back(filePath);
	}
	return textures;
}
...
std::vector<sf::Image> makeBoardIcons(sf::Image& boardSpriteSheet, sf::RenderTexture& renderTexture, sf::Sprite& iconTexture)
{
	...
	for (int i = 0; i < 24; i++) {
		int x = i % 4;
		int y = std::floor(i / 4.0f);
		sf::Texture boardTexture(boardSpriteSheet, false, sf::IntRect({ x * 1024, y * 1024 }, { 1024, 1024 }));
		sf::Sprite currentBoardC{ boardTexture, sf::IntRect{ {0, 0}, {128 * 3, 128 * 3} } };
		...
		icons.push_back(renderTexture.getTexture().copyToImage());
	}
	return icons;
}
...
{
	std::shared_ptr<Board> board = std::make_shared<Board>(boardSettings, stockfishSettings, windowSize, pieceSpriteSheets, boardSpriteSheet, textFont, boardTexture, pieceTextures, boardTextures, soundBuffers, false);
...
GUIBoard::GUIBoard(const tgui::Picture::Ptr& boardPicture, tgui::Gui& gui, std::vector<tgui::Texture>& pieceTextures) : gui(&gui)
{
	boardScale = boardPicture->getSize().x / 1024.0f;
	pieceScale = boardScale * (128.0f / 320.0f);
	boardMultiplier = boardScale * 128.0f;
	boardSize = boardPicture->getSize();
	boardOffset = boardPicture->getAbsolutePosition();
}

void GUIBoard::setPieces(std::string fen, std::vector<tgui::Texture>& pieceTextures, bool reversed)
{
	gui->removeAllWidgets();
	previewPieces = generatePositionFromFENID(fen, pieceTextures, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
...
// Black --> White
std::vector<SVGPiece> generatePositionFromFENID(std::string code, std::vector<tgui::Texture>& pieceTextures, float pieceScale, sf::Vector2f boardOffset, sf::Vector2f boardSize, float boardMultiplier, bool reversed) {
	...
	for (int i = 0; i < ranks.size(); i++) {
		...
		for (int j = 0; j < rank.size(); j++) {
			...
			if (std::isdigit(letter)) {
			...
			else if (letter != '~' && Chess::isValidSquare({ x, y })) {
				...
				case 'b':
				{
					pieces.emplace_back(pieceTextures.at(0 + offset), x, y, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
					break;
				}
				case 'k':
				{
					pieces.emplace_back(pieceTextures.at(1 + offset), x, y, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
					break;
				}
				case 'n':
				{
					pieces.emplace_back(pieceTextures.at(2 + offset), x, y, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
					break;
				}
				case 'p':
				{
					pieces.emplace_back(pieceTextures.at(3 + offset), x, y, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
					break;
				}
				case 'q':
				{
					pieces.emplace_back(pieceTextures.at(4 + offset), x, y, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
					break;
				}
				case 'r':
				{
					pieces.emplace_back(pieceTextures.at(5 + offset), x, y, pieceScale, boardOffset, boardSize, boardMultiplier, reversed);
					break;
				}
				}
				x++;
...
SVGPiece::SVGPiece(const tgui::Texture& texture, int x, int y, float scale, sf::Vector2f boardOffset, sf::Vector2f boardSize, float boardMultiplier, bool reversed) : pieceImage(tgui::Picture::create(texture))
{
	pieceImage->setSize({ boardMultiplier, boardMultiplier });
	pieceImage->setOrigin(0.5f, 0.5f);
	pieceImage->setPosition(tgui::Layout2d{ Chess::getGlobalPosition(sf::Vector2i{x, y}, boardOffset, boardSize, boardMultiplier, reversed) });
}
